...
int main(){
  int H,W; cin>>H>>W;
  v2i32 P(H,v1i32(W,0)); rep(h,H)rep(w,W) cin>>P[h][w];
  int res = 0;
  rep_bit(bit,H){
    if (bit==0) continue;
    v1i32 ls;
    rep(i,H){
     if (bit&(1<<i)) ls.push_back(i);
    }
    map<int,int> dict;
    rep(w,W){
      int cand = P[ls[0]][w];
      bool flg = true;
      for (auto& h:ls){
        if (P[h][w]!=cand) flg = false;
      }
      if (!flg) continue;
      if (!dict.count(cand)) dict[cand] = ls.size();
      else dict[cand] += ls.size();
    }
    dump(ls);
    dump(dict);
    int tmp = 0;
