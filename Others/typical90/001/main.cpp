...
bool isOK(auto &ls, int mid, int K){
  // 切ってもmidを満たせない場合
  rep(i,ls.size()-1){
    if (i==0){
      if (ls[i]>mid) return false;
    }
    else{
      if (ls[i+1]-ls[i]>mid) return false;
    }
  }
  int cnt = 0;
  int now = 0;
...
int main(){
  int N,L; cin>>N>>L;
  int K; cin>>K;
  v1i32 A(N+1,0); rep(i,N) cin>>A[i+1];
  A.push_back(L);
  v1i32 B(N,0);
