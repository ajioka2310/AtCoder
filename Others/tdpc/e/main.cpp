...
int main(){
  ...
  dump(digit);
  // N桁 / flg / MOD D
  v3i64 dp(N+1,v2i32(2,v1i32(0,100)));
  dp[0][0][0] = 1;
  rep(i,1,n)rep(flg,2)rep(x,10)rep(j,D){
    // i+1桁目に対してi桁目がflgであり、i+1桁目がxであるときの状態をMOD_Dごとに走査
    // i -> i+1桁目への遷移をイメージ
    // dp[i][flg][j]がそれぞれi+1桁目がxだった場合にどこに遷移するかを考える。
    // 
    if (flg && x>N[i]) continue;
    else if (flg && x==N[i]){
      // i桁目までがNと一致していて、i+1桁も一致する状態
      dp[i+1][1][(j+x)%D] += dp[i][flg][j];
    }
    else if (flg && x<N[i]){
      // i桁目までがNと一致していて、i+1桁目は一致しないケース
      dp[i+1][0][(j+x)%D] += dp[i][flg][j];
    }
    else{
      // i桁目までがNと一致していないケース;
