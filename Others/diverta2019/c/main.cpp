...
int main(){
  int N; cin>>N;
  int res=0,res_a=0,res_b=0,res_c=0;
  rep(i,N){
    string S; cin>>S;
    rep(j,S.size()-1){
      if (S[j]=='A'&&S[j+1]=='B') res++;
    }
    if (S[0]=='B'&&S[S.size()-1]!='A') res_b++;
    if (S[0]!='B'&&S[S.size()-1]=='A') res_a++;
    if (S[0]=='B'&&S[S.size()-1]=='A') res_c++;
  }
  dump(res,res_a,res_b);
  if (res_c==0){
    cout << res+min(res_a,res_b) << endl;
  }
  else{
    if (res_a+res_b>0){
      cout << res+min(res_a,res_b)+res_c << endl;
    }
    else{
      cout << res+res_c-1 << endl;
    }
  }
}
