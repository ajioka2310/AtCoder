...
int main(){
  ...
  sort(all(X));
  int idx = lower_bound(all(X),0) - X.begin();
  dump(idx,K,X|cp::index());
  i64 res = 1e10;
  rep(i,N-(K-2)){
    int left = i;
    int right = i+(K-1);
    ...
    if (left<=idx&&idx<=right){
      if (right==idx) res = min(1LL*abs(X[left]),res);
      else if (left==idx) res = min(1LL*abs(X[right]),res);
      else {
        res = min(res,1LL*abs(X[left])*2+1LL*abs(X[right]));
        res = min(res,1LL*abs(X[right])*2+1LL*abs(X[left]));
      }
    }
    else continue;
    dump(res);
