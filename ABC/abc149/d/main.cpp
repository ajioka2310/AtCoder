...
int main(){
  ...
  rep(i,T.size()){
    if (i<K){
      ...
      if (T[i]=='r'&&T[i-K]!='r') res+= P;
      if (T[i]=='r'&&T[i-K]=='r'&&ls[i-K]) res+= P;
      if (T[i]=='r'&&T[i-K]=='r'&&ls[i-K]==0) ls[i]=1;

      if (T[i]=='s'&&T[i-K]!='s') res+= R;
      if (T[i]=='s'&&T[i-K]=='s'&&ls[i-K]) res+= R;
      if (T[i]=='s'&&T[i-K]=='s'&&ls[i-K]==0) ls[i]=1;

      if (T[i]=='p'&&T[i-K]!='p') res+= S;
      if (T[i]=='p'&&T[i-K]=='p'&&ls[i-K]) res+= S;
      if (T[i]=='p'&&T[i-K]=='p'&&ls[i-K]==0) ls[i]=1;
    }
    dump(res);
  }
  dump(T);
