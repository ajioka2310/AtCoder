...
int main(){
  i64 H,W,Y; cin>>H>>W>>Y;
  v2i64 A(H,v1i64(W,0)); rep(i,H){rep(j,W) cin>>A[i][j];}
  v2i64 B(H,v1i64(W,0));
  ...
  krep(now,1,Y+1){
    while(!que.empty()){
      ...
      if (h<=now){
        ...
        // up
        if (0<point.first&&B[point.first-1][point.second]==0){
          que.push({A[point.first-1][point.second],{point.first-1,point.second}});
          B[point.first-1][point.second] = 1;
        }
        // down
        if (point.first<H-1&&B[point.first+1][point.second]==0){
          que.push({A[point.first+1][point.second],{point.first+1,point.second}});
          B[point.first+1][point.second] = 1;
        }
        // left
        if (0<point.second&&B[point.first][point.second-1]==0){
          que.push({A[point.first][point.second-1],{point.first,point.second-1}});
          B[point.first][point.second-1] = 1;
        }
        // right
        if (point.second<W-1&&B[point.first][point.second+1]==0){
          que.push({A[point.first][point.second+1],{point.first,point.second+1}});
          B[point.first][point.second+1] = 1;
        }
        dump(B);
