...
int main(){
  ...
  rep(i,N){
    ...
    cin >> a >> b >> c;
    b++;
    if (!dict.contains(a)) dict[a] = c;
    else dict[a]+=c;
    if (!dict.contains(b)) dict[b] = -c;
    else dict[b]-=c;
  }

  vector<pair<i64,i64>> ls;
  for (auto &[key,value]: dict){
    ls.push_back({key,value});
  }
  dump(ls);
  pair<i64,i64> now = {ls[0].first,ls[0].second};
  i64 res = 0;
  krep(i,1,ls.size()){
    pair<i64,i64> next = {ls[i].first,ls[i].second};
    res += 1LL*(next.first-now.first)*min(now.second,1LL*C);
    now = {next.first,now.second+next.second};
    dump(res);
    dump(now);
  }
  cout << res << endl;
