...
int main(){
  int N,S,T; cin>>N>>S>>T;
  v2i32 ls(N,v1i32(4,0)); rep(i,N)rep(j,4) cin>>ls[i][j];
  v1i32 idx;rep(i,N)idx.push_back(i);
  ...
  do{
    ...
    rep_bit(bit,N){
      ...
      rep(i,N){
        if (bit&1<<i){
          view.push_back(1);
          pair_int next = {ls[idx[i]][0],ls[idx[i]][1]};
          res += sqrt(pow(now.first-next.first,2)+pow(now.second-next.second,2))/S;

          now = next;
          next = {ls[idx[i]][2],ls[idx[i]][3]};
          res += sqrt(pow(now.first-next.first,2)+pow(now.second-next.second,2))/T;
          now = next;
        }
        else{
          view.push_back(0);
          pair_int next = {ls[idx[i]][2],ls[idx[i]][3]};
          res += sqrt(pow(now.first-next.first,2)+pow(now.second-next.second,2))/S;
          now = next;
          next = {ls[idx[i]][0],ls[idx[i]][1]};
          res += sqrt(pow(now.first-next.first,2)+pow(now.second-next.second,2))/T;
          now = next;
        }
      }
