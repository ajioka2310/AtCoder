...
int main(){
  ...
  while(ng-ok>1){
    ...
    rep(i,N){
      ...
      rep(j,A[i]+1){
        if (mid-B[i]*j<=0){
          price = min(price,Q[i]*j);
        }
        else{
          price = min(price,Q[i]*j+(mid-B[i]*j + A[i]-1)/A[i]*P[i]) ;
        }
      }
      rep(j,B[i]+1){
        if (mid-A[i]*j<=0){
          price = min(price,P[i]*j);
        }
        else{
          price = min(price,P[i]*j+(mid-A[i]*j + B[i]-1)/B[i]*Q[i]) ;
        }
      }
      dump(i,price);
      now -= price;
