...
i64 score(auto &ls, auto &A){
  i64 res = 0;
  rep(i,ls.size()){
    if (A[ls[i][1]-1]-A[ls[i][0]-1]==ls[i][2]) res += ls[i][3];
  }
  return res;
}


i64 dfs(auto &ls, v1i32 A, int M, int N){
  if (A.size()==N){
    dump(A);
    return score(ls, A);
  }
...
int main(){
  int N,M,Q; cin>>N>>M>>Q;
  v2i32 ls(Q,v1i32(4,0)); rep(i,Q){rep(j,4) cin>>ls[i][j];}
  dump(ls);
  int res = dfs(ls,{},M,N);
  cout << res << endl;
