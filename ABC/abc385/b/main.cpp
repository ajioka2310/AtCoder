...
int main()
{
	...
	VV(char,ls,H,W);
	STR(T);
	v2i64 memo(H,v1i64(W,0));
	pair<i64,i64> now = {X-1,Y-1};
	i64 cnt = 0;
	rep(i,si(T)){
		pair<i64,i64> next;
		if (T[i]=='U') next = {now.fi-1, now.se};
		if (T[i]=='D') next = {now.fi+1, now.se};
		if (T[i]=='L') next = {now.fi, now.se-1};
		if (T[i]=='R') next = {now.fi, now.se+1};
		if (next.fi<0||next.fi>H||next.se<0||next.se>W) continue;
		if (ls[next.fi][next.se]=='#') continue;
		if (ls[next.fi][next.se]=='@'&&memo[next.fi][next.se]==0){
			cnt++;
			memo[next.fi][next.se] = 1;
		} 
		now = next;
	}
	cout << now.first+1 << " " << now.second+1 << " " << cnt << endl;
