...
int main()
{
	...
	map<i64,set<i64>> rows,cols;
	rep(i,N){
		rows[ls[i].se].insert(ls[i].fi);
		cols[ls[i].fi].insert(ls[i].se);
	}

	i64 now_x = Sx, now_y = Sy;
	i64 res = 0;

	rep(i,M){
		i64 next_x = now_x,next_y = now_y;
		dump(rows);
		dump(cols);

		if (D[i]=='U'){
			next_y += C[i];
			auto lower = cols[now_x].lower_bound(min(now_y,next_y));
			auto upper = cols[now_x].upper_bound(max(now_y,next_y));

			v1i64 visitedY;
			visitedY.reserve(distance(lower, upper));
			for(auto it = lower; it != upper; ++it){
					visitedY.push_back(*it);
			}
			res += 1LL*visitedY.size();
			cols[now_x].erase(lower,upper);
			rep(j,si(visitedY)){
					rows[visitedY[i]].erase(now_x);
			}
		}
		if (D[i]=='D'){
			next_y -= C[i];
		...
		if (D[i]=='R'){
			next_x += C[i];
			auto lower = rows[now_y].lower_bound(min(now_x,next_x));
			auto upper = rows[now_y].upper_bound(max(now_x,next_x));
			v1i64 visitedX;
			visitedX.reserve(distance(lower, upper));
			for(auto it = lower; it != upper; ++it){
					visitedX.push_back(*it);
			}
			res += 1LL*visitedX.size();
			rows[now_y].erase(lower,upper);
			rep(j,si(visitedX)){
					cols[visitedX[i]].erase(now_y);
			}
		}
		if (D[i]=='L'){
			next_x -= C[i];
