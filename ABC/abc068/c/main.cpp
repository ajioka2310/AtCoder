...
void dfs(auto &ls, auto &dist, int node, int depth){
  dist[node] = depth;
  for (auto &next:ls[node]){
    dump(node,next,dist[next],depth);
    if (dist[next]<=depth) continue;
    else{
      dfs(ls,dist,next,depth+1);
    }
  }
}
...
int main(){
  ...
  bool flg = false;
  krep(i,1,N-1){
    if (ls[i].contains(0)&&ls[i].contains(N-1)) flg = true;
  }
