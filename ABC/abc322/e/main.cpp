...
int main(){
  ...
  v1i64 C(N);
  v2i64 A(N,v1i64(5,5));
  v6i64 dp(N+1,v5i64(6,v4i64(6,v3i64(6,v2i64(6,v1i64(6,INF))))));
  rep(j,6)rep(k,6)rep(l,6)rep(m,6)rep(n,6){
    dp[0][j][k][l][m][n] = 0;
  }
  rep(i,N)rep(j,6)rep(k,6)rep(l,6)rep(m,6)rep(n,6){
    if (A[i][0]<j||A[i][1]<k||A[i][2]<l||A[i][3]<m||A[i][4]<n){
      dp[N+1][j][k][l][m][n] = dp[N][j][k][l][m][n];
    }
    dp[N+1][j][k][l][m][n] = 0;
  }
