...
int main(){
  ...
  dump(res);
  // case 2
  // i64 N_sq = int(sqrt(N)+1);
  deque<i64> que = primes(2020202);
  dump(que.size());
  i64 now = que.front();que.pop_front();
  while(!que.empty()){
    i64 end = que.back();
    if (end*end*now*now<=N){
      // dump(now);
      // dump(end);
      // dump(que.size());
      // dump(que);
