...
void dfs(int node, int parent, int parent_color, vector<vector<pair_int>> &G, v1i32 &res){
  int color = 1;
  for(auto [next_node, idx] : G[node]){
    if(next_node == parent) continue;
    if(color == parent_color) color++;
    res[idx] = color;
    dfs(next_node, node, color, G, res);
    color++;
  }
}
