...
int main(){
  ...
  rep(i,N) cin>>ls[i].first>>ls[i].second;

  v3i64 dp(N+1,v2i64(N+1,v1i64(X+1,INF)));
  // rep(i,N)rep(j,X+1){
  //   dp[i][0][j] = 0;
  // }
  // rep(i,N)  dp[i][1][ls[i].first] = ls[i].second;
  dp[0][0][0] = 0;
  krep(i,1,N+1){
    i64 A = ls[i-1].first;
    i64 B = ls[i-1].second;
    rep(l,N+1){
      rep(j,X+1){
        if (0<=j-A&&j-A<=X){
          if (l>0) chmin(dp[i][l][j],dp[i-1][l-1][j-A]+B);
          chmin(dp[i][l][j],dp[i-1][l][j]);
        }
        else {
          chmin(dp[i][l][j],dp[i-1][l][j]);
        }
