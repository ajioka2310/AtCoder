...
int main()
{
	...
	vector<pair<ll,ll>> Y;
	rep(i,N){
		Y.pb({ls[i].fi+ls[i].se*T,ls[i].se});
	}
	vector<pair<ll,ll>> right;
	vector<pair<ll,ll>> left;
	rep(i,N){
		if(ls[i].se==1) right.pb({ls[i].fi,Y[i].fi});
		else left.pb({ls[i].fi,Y[i].fi});
	}
	dump(right);
	dump(left);
	...
	ll idx = 0;
	ll res = 0;
	// rep(i,si(left)) que.pb(left[i]);

	fore(st,ed,right){
		while(que.size()){
			if (st>que.front().fi) que.ppf();
			else break;
		}
		while(idx<si(left)){
			if (st<left[idx].fi&&ed>=left[idx].se){
				que.pb(left[idx]);
				idx++;
			}
			else if (st>left[idx].fi) idx++;
			else break;
		}
		dump(st,ed);
		dump(idx);
