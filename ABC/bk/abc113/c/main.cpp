...
int main(){
  int N,M; cin>>N>>M;
  vector<pair_int> ls(M);
  rep(i,M){
    int P,Y; cin>>P>>Y;
    ls[i] = {P,Y};
  }
  map<int,vector<pair_int>> dict;
  rep(i,M){
    dict[ls[i].first].push_back({ls[i].second,0});
  }

  map<pair_int,int> dict_fix;
  for (auto &[key,value]:dict){
    sort(all(value));
    rep(i,value.size()){
      dict_fix[{key,value[i].first}] = i+1;
    }
  }

  dump(dict_fix);
  rep(i,M){
    printf("%06d%06d\n",ls[i].first,dict_fix[{ls[i].first,ls[i].second}]);
  }
}
