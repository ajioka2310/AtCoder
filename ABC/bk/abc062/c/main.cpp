...
int main(){
  ...
  if (H>=3){
    if (H%3==0) res = 0;
    else if (H%3==1) res = min(res,W);
    else if (H%3==2) res = min(res,W);
  }

  // WW
  if (W>=3){
    if (W%3==0) res = 0;
    else if (W%3==1) res = min(res,H);
    else if (W%3==2) res = min(res,H);
  }

  // HW
  krep(i,1,(W+1)/2+1){
    i64 S1 = H*i;
    if (H%2==0) res = min(res,abs(S1-(W-i)*H/2));
    else {
      i64 S2 = (W-i)*(H+1)/2;
      i64 S3 = (W-i)*(H-1)/2;
  ...
  krep(i,1,(H+1)/2+1){
    i64 S1 = W*i;
    if (W%2==0) res = min(res,abs(S1-(H-i)*W/2));
    else {
      i64 S2 = (H-i)*(W+1)/2;
      i64 S3 = (H-i)*(W-1)/2;
      res = min(res,max({S1,S2,S3})-min({S1,S2,S3}));
