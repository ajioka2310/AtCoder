...
int main(){
  int N; cin>>N;
  v3i64 dp(N+1,v2i64(N+1,v1i64(N+1,0)));
  rep(i,N)rep(j,N)rep(k,N){
    i64 A; cin>>A;
    dp[i+1][j+1][k+1] = A;
  }
  krep(i,1,N+1)rep(j,N+1)rep(k,N+1){
    dp[i][j][k] += dp[i-1][j][k];
  }
  rep(i,N+1)krep(j,1,N+1)rep(k,N+1){
    dp[i][j][k] += dp[i][j-1][k];
  }
  rep(i,N+1)rep(j,N+1)krep(k,1,N+1){
    dp[i][j][k] += dp[i][j][k-1];
  }
