...
int main(){  
  ...
  rep(a,(F/100*A)+1){
    rep(b,((F-100*A*a)/B)+1){
      rep(c,(F-100*A*a-100*B*b)/C+1){
        rep(d,(F-100*A*a-100*B*b-C*c)/D+1){
          double p = (100.0*(C*c+D*d)/(100*A*a+100*B*b+C*c+D*d));
          dump(a,b,c,d,p);

          if (now_p<=p&&p<=100.0*E/(100.0+1.0*E)){
            dump(a,b,c,d);
            now_p = p;
            now_w = 100*A*a+100*B*b;
            now_s = C*c+D*d;
          }
        }
      }
