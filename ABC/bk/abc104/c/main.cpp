...
int main(){
  ...
  int res = INF;
  rep_bit(bit,D){
    pair<int,i64> now = {0,0}; 
    v1i32 memo(D,0);
    rep(i,D){
      if (bit&(1<<i)){
        memo[i] = 1;
        now.first += ls[i].first;
        now.second += 1LL*ls[i].first*(i+1)*100+1LL*ls[i].second;
      } 
    ...
    dump(now);
    // check1
    if (now.second>=G) res = min(res,now.first);
    else {
      rrep(i,D){
        if (memo[i]==1);
        else{
          if (G-now.second>(ls[i].first-1)*(i+1)*100){
            now.first += ls[i].first-1;
            now.second += (ls[i].first-1)*(i+1)*100;
          }
          else {
            int tmp =  (G-now.second+(i+1)*100-1)/((i+1)*100);
            dump(tmp);
            now.first += tmp;
            res = min(res,now.first);
            break;
          }
        }
      }
