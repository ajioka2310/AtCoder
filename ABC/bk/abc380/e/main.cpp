...
int main(){
  int N,Q; cin>>N>>Q;
  // 各色のマスが何個あるかを表すvec C
  // 連結成分の左端のマスの番号を持つset S
  // それらのますが何色に塗られているかを持つ dict D
  v1i32 C(N,1);
  set<int> S;
  ...
  rep(i,Q){
    ...
    if (div==1){
      ...
      C[D[L]] += R-L;
      // 右側境界消す
      if (D[L]==D[R]) S.erase(R);
      // 左側境界消す
      if (L==*S.begin()) continue;
      auto iter_l = S.lower_bound(L);
      int L_ = *prev(iter_l);
      if (D[L]==D[L_]) S.erase(L);
