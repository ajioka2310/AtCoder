...
int main(){
  v1i32 ls(3,0); rep(i,3) cin>>ls[i];
  sort(all(ls));
  if (ls[0]==ls[1]&&ls[1]==ls[2]){
    cout << 0 << endl;
  }
  else if (ls[1]==ls[2]){
    if ((ls[1]-ls[0])%2==0) cout << (ls[1]-ls[0])/2 << endl;
    else cout << 1+ (ls[1]+1-ls[0])/2 << endl;
  }
  else {
    int sub1 = ls[2] - ls[1];
    int sub2 = ls[2] - ls[0];
    if ((sub2-sub1)%2==0) cout << sub1+(sub2-sub1)/2 << endl;
    else {
      sub1++;
      cout << sub1+(sub2-sub1)/2 +1 << endl;
    }
  }
}
