...
int main(){
  ...
  rep(i,4)sort(all(ls[i]));
  rep(i,4)krep(j,1,ls[i].size()) ls[i][j] += ls[i][j-1];

  dump(ls);
  i64 res = -1LL*1e10;
  dump(res);
  rep(a,ls[0].size())rep(b,ls[1].size())rep(c,ls[2].size())rep(d,ls[4].size()){
    if (1LL*a*(w1)+1LL*b*(w1+1)+1LL*c*(w1+2)+1LL*d*(w1+3)<=1LL*W){
      res = max(res,1LL*ls[0][a]+1LL*ls[1][b]+1LL*ls[2][c]+1LL*ls[3][d]);
      dump(a,b,c,d,res);
