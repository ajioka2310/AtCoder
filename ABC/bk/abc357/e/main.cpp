...
v1i32 A(202020,0);
vector<set<int>> ls(202020); // 202020

void dfs(int st, int now, set<int> now_set){
  dump(st,now,now_set);
  if (now_set.contains(A[now])){
    ls[st] = now_set;
    return ;
  } 
  else if (A[now]<now&ls[A[now]].size()>0){
    now_set.insert(all(ls[A[now]]));
    ls[st] = now_set;
    return ;
  }
  else{
    now_set.insert(A[now]);
    dfs(st,A[now],now_set);
  }
}
