...
int main(){
  ...
  if (S[N-1]=='0') ls.push_back(0);
  dump(ls);

  vec ls_cumsum(ls.size()+1,0);
  rep(i,ls.size()) ls_cumsum[i+1] = ls_cumsum[i] + ls[i];

  int res = 0;
  for (int left=0; left<ls_cumsum.size(); left+=2){
    int right = left + K*2+1;
    if (right>=ls_cumsum.size()) right = ls_cumsum.size() -1;
