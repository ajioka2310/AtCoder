...
int main(){
  ...
  rep(i,Q){
    ...
    int idx_upper = upper_bound(all(A),K) - A.begin();
    int K_fix = K + idx_upper;
    int idx_upper_fix = upper_bound(all(A),K_fix) - A.begin();

    while(idx_upper!=idx_upper_fix){
      idx_upper = idx_upper_fix;
      idx_upper_fix = upper_bound(all(A),K+idx_upper) - A.begin();
    }
    cout << K+idx_upper_fix << endl;
  }
