...
CPP_DUMP_DEFINE_EXPORT_OBJECT(Edge, to, cost);

// ダイクストラ
void Dijkstra(const Graph& graph, v1i64& distances, int startIndex, v1i32& path){
    pqasc<Pair> q;
...
int main(){
  int H,W; cin>>H>>W;
  Graph graph(H*W);
  v1i64 distances(H*W,INF);
  v1i32 path(H*W,-1);
  vector<vector<char>> ls(H,vector<char>(W,'.'));
  rep(i,H)rep(j,W){
    char a; cin>>a;
    if (a=='#') ls[i][j] = a;
  }
  ...
  rep(i,H)rep(j,W){
    // up down left right
    if (i>0&&ls[i-1][j]=='.') graph[i*W+j].push_back({(i-1)*W+j,0});
    if (i<H-1&&ls[i+1][j]=='.') graph[i*W+j].push_back({(i+1)*W+j,0});
    if (j>0&&ls[i][j-1]=='.') graph[i*W+j].push_back({(i)*W+(j-1),0});
    if (j<W-1&&ls[i][j+1]=='.') graph[i*W+j].push_back({(i)*W+(j+1),0});

    // up-left up-right
    krep(k,-2,3)krep(l,-2,3){
      if (k==0&&l==0) continue;
      if (k==-2&&l==-2) continue;
      if (k==-2&&l==2) continue;
      if (k==2&&l==-2) continue;
      ...
      if (0<=i-k&&i-k<H&&0<=j-l&&j-l<W){
        graph[i*W+j].push_back({(i-k)*W+(j-l),1});
      }
    }
  }
  Dijkstra(graph,distances,0,path);
  cout << distances[H*W-1] << endl;
  dump(graph);
