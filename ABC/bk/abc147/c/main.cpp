...
int main(){
  ...
  rep_bit(bit,N){
    ...
    rep(i,N){
      for (auto &now: ls[i]){
        if (cand[i] == -1) continue; // Skip if the person is not honest
        if (cand[now.first] == -1) cand[now.first] = now.second; // Assign if not yet assigned
        else if (cand[now.first] != now.second) flg = false; // Contradiction found
      }
    }
