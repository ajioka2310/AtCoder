...
int main(){
  int N,M,Q; cin>>N>>M>>Q;
  vector<pair_int> baggage(N);
  rep(i,N){
    cin>>baggage[i].first>>baggage[i].second;
  }
  v1i32 X(M,0); rep(i,M) cin>>X[i];
  dump(baggage);  
  rep(i,Q){
    int L,R; cin>>L>>R;
    L--;R--;
    ...
    v1i32 memo(N,0);
    v1i32 Y;
    rep(i,M) if (i<L||R<i) Y.push_back(X[i]);
    sort(all(Y));
    rep(i,Y.size()){
      int size = Y[i];
      int value_max = 0;
      int idx_max = -1;
      rep(j,N){
        if (memo[j]==0&&baggage[j].first<=size&&baggage[j].second>=value_max){
          value_max = baggage[j].second;
          idx_max = j;
        }
      }
