...
int main()
{
	...
	ll res = INF;
	// yoko
	v2i64 yoko_x(H,v1i64(W+1,0));
	v2i64 yoko_d(H,v1i64(W+1,0));
	rep(i,H){
		rep(j,W){
			if(ls[i][j]=='x') yoko_x[i][j+1] = 1;
			if(ls[i][j]=='.') yoko_d[i][j+1] = 1;
		}
		rep(j,W){
			yoko_x[i][j+1] += yoko_x[i][j];
			yoko_d[i][j+1] += yoko_d[i][j];
		}
		if (W<K) continue;
		rep(j,W-K+1){
			if (yoko_x[i][j+K]-yoko_x[i][j]==0){
				chmin(res,yoko_d[i][j+K]-yoko_d[i][j]);
			}
		}
	}

	// tate
	v2i64 tate_x(H+1,v1i64(W,0));
	v2i64 tate_d(H+1,v1i64(W,0));
	rep(j,W){
		rep(i,H){
			if(ls[i][j]=='x') tate_x[i+1][j] = 1;
			if(ls[i][j]=='.') tate_d[i+1][j] = 1;
		}
		rep(i,H){
			tate_x[i+1][j] += tate_x[i][j];
			tate_d[i+1][j] += tate_d[i][j];
		}
		if (H<K) continue;
		rep(i,H-K+1){
			if (tate_x[i+K][j]-tate_x[i][j]==0){
				chmin(res,tate_d[i+K][j]-tate_d[i][j]);
			}
		}
	}
	dump(H,K);
