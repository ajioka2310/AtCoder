...
int main(){
  int N; cin>>N;
  string S; cin>>S;
  v3i32 dp(N+1,v2i32(1<<10,v1i32(11,0)));
  // init
  dp[0][0][10] = 1;
  ...
  krep(i,0,N){
    rep_bit(bit,10){
      rep(last,11){
        ...
        if (dp[i][bit][last]!=0){
          // 連続パターン
          dp[i+1][bit][last] += dp[i][bit][last];
          // ビット追加パターン
          int next = S[i] - 'A';
          // すでに選ばれていて、連続でないパターンはとばす。
          if ((bit&(1<<next))&& last!=next) continue;
          // 今回追加
          dp[i+1][bit|(1<<next)][next] += dp[i][bit][last];
        }
      }
    }
  }
  i64 ans = 0;
  rep_bit(bit,10)rep(last,11){
    ans += dp[N][bit][last];
    ans = ans % MOD;
  }
  cout << ans << endl;
