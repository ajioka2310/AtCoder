...
int main(){
  ...
  dsu ufd(N+1);
  //
  vector<pqdesc<int>>  ls(N+1,pqdesc<int>({}));
  rep(i,N+1){
    ls[i].push(i);
  }
  //
  rep(i,Q){
    ...
    if (a==1){
      int u,v; cin>>u>>v;
      if (!ufd.same(u,v)){
        int led_u = ufd.leader(u);
        int led_v = ufd.leader(v);
        ufd.merge(u,v);
        int led = ufd.leader(u);
        dump(led);

        if (led!=led_u){
          while(ls[led_u].size()){
            ls[led].push(ls[led_u].top());
            ls[led_u].pop();
          }
        }
        if (led!=led_v){
          while(ls[led_v].size()){
            ls[led].push(ls[led_v].top());
            ls[led_v].pop();
          }     
        }
      }
      ...
      int v,k; cin>>v>>k;
      int led = ufd.leader(v);
      dump(v,k,led,ls[led].size());

      
      v1i32 hold;
      if (ls[led].size()>=k){
        rep(i,k-1){
          hold.push_back(ls[led].top());
          ls[led].pop();
        }
        dump("ans");        
        cout << ls[led].top() << endl;
