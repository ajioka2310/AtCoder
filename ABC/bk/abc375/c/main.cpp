  ...
  rep(i,N/2){
    ...
    if (rot==0){
      ...
      krep(j,i,N-i-1){
        ...
        res[j][N-i-1]   = ls[i][j];
        // 0,N-1 -> N-2, N-1
        res[N-i-1][N-j-1] = ls[j][N-i-1];
        // N-1,N-1 -> 1, 0
        res[N-j-1][i]   = ls[N-i-1][N-j-1];
        //
        res[i][j]       = ls[N-j-1][i];
      }
    }
    if (rot==1){
      krep(j,i,N-i-1){
        res[N-i-1][N-j-1]   = ls[i][j];
        res[i][j] = ls[N-i-1][N-j-1];
        res[N-j-1][i]   = ls[j][N-i-1];
        res[j][N-i-1]       = ls[N-j-1][i];
      }
    }
    if (rot==2){
      krep(j,i,N-i-1){
        res[N-j-1][i]   = ls[i][j];
        res[i][j] = ls[j][N-i-1];
        res[j][N-i-1]   = ls[N-i-1][N-j-1];
        res[N-i-1][N-j-1]       = ls[N-j-1][i];
      }
    }
    if (rot==3){
      krep(j,i,N-i-1){
        ...
        res[i][j]   = ls[i][j];
        res[j][N-i-1] = ls[j][N-i-1];
        res[N-i-1][N-j-1]   = ls[N-i-1][N-j-1];
        res[N-j-1][i]       = ls[N-j-1][i];
      }
    }
  }
  dump(ls);
