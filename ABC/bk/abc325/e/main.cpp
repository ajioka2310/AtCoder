...
int main(){
  ...
  rep(i,N)rep(j,N){
    Edge now1 = {j,A*D[i][j]};
    graph1[i].pb(now1);
    Edge now2 = {j,B*D[i][j]+C};
    graph2[i].pb(now2);
  }
  v1i64 distances1(N,INF),distances2(N,INF);
  v1i64 path1(N,-1),path2(N,-1);
  Dijkstra(graph1,distances1,0,path1);
  Dijkstra(graph2,distances2,N-1,path2);

  i64 res = INF;
  rep(i,N){
    chmin(res,distances1[i]+distances2[i]);
  }
  cout << res << endl;
}
