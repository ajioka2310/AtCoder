...
int main(){
  i64 N,M; cin>>N>>M;
  vector<vector<pair<pair<i64,i64>,pair<i64,i64>>>> ls(N);
  vector<pair<i64,i64>> res(N,pair<i64,i64>{INF,INF});
  res[0] = {0,0};
  rep(i,M){
    i64 A,B,X,Y; cin>>A>>B>>X>>Y;
    A--;B--;
    ls[A].push_back({{A,B},{X,Y}});
    ls[B].push_back({{B,A},{-X,-Y}});
  }
  deque<pair<pair<i64,i64>,pair<i64,i64>>> que;
  rep(i,ls[0].size()){
    que.push_back(ls[0][i]);
  }
  while(!que.empty()){
    auto [idx,cond] = que.front();que.pop_front();
    if (res[idx.second].first != INF) continue;
    res[idx.second] = {res[idx.first].first+cond.first,res[idx.first].second+cond.second};
    rep(i,ls[idx.second].size()){
      que.push_back(ls[idx.second][i]);
    }
  }
  rep(i,N){
    if (res[i].first==INF){
