...
int main(){
  string S; cin>>S;
  v2i64 dp(8,v1i64(S.size(),0));
  string ans = "chokudai";
  if (S[0]=='c') dp[0][0] = 1;

  krep(i,1,S.size()){
    if (S[i]=='c') dp[0][i] = dp[0][i-1]+1;
    else dp[0][i] = dp[0][i-1];
  }

  krep(i,1,ans.size()){
    krep(j,1,S.size()){
      if (S[j]==ans[i]) dp[i][j] = (dp[i-1][j-1]+dp[i][j-1])%MOD;
      else dp[i][j] = dp[i][j-1];
    }
  }
  dump(dp);
  dump(dp[ans.size()-1][S.size()-1]);
  cout << dp[ans.size()-1][S.size()-1] << endl;
}
