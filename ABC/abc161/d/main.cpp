...
int main(){
  ...
  v2i64 ls(12);
  // init
  krep(i,1,10) ls[1].push_back(i);
  i64 now = ls[1].size();
  i64 digit = 1;
  while(now<K){
    rep(i,ls[digit].size()){
      if (ls[digit][i]%10==0){
        ls[digit+1].push_back(ls[digit][i]*10+ls[digit][i]%10);
        ls[digit+1].push_back(ls[digit][i]*10+ls[digit][i]%10+1);
      }
      else if (ls[digit][i]%10==9){
        ls[digit+1].push_back(ls[digit][i]*10+ls[digit][i]%10);
        ls[digit+1].push_back(ls[digit][i]*10+ls[digit][i]%10-1);
      }
      else {
  ...
  v1i64 res;
  rep(i,ls.size())rep(j,ls[i].size()) res.push_back(ls[i][j]);
  sort(all(res));
  dump(res);
