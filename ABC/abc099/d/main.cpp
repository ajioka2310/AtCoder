...
int main(){
  int N,C; cin>>N>>C;
  v2i32 D(C,v1i32(C,0)); rep(i,C){rep(j,C) cin>>D[i][j];}
  v2i32 c(N,v1i32(N,0)); rep(i,N){rep(j,N) cin>>c[i][j];}
  v2i32 memo(3,v1i32(C,0));
  rep(color, C){
    rep(i,N)rep(j,N){
      memo[(i+j+2)%3][color] += D[c[i][j]-1][color];
    }
  }
  dump(memo);
  int res = INF;
  rep(i,C)rep(j,C)rep(k,C){
    if (i!=j&&j!=k&&k!=i) res = min(res,memo[0][i]+memo[1][j]+memo[2][k]);
  }
  cout << res << endl;
