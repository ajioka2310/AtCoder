...
int main(){
  ...
  rep(i,M){
    ...
    set.insert({b,a});
    rep(i,8){
      i64 a_fix = a+base[i].first;
      i64 b_fix = b+base[i].second;
      if (1<=a_fix&&a_fix<=N&&1<=b_fix&&b_fix<=N){
        set.insert({b_fix,a_fix});
      }
    }
    dump(set.size());
  }
