...
int main(){
  ...
  krep(i,2,N+1){
    ...
    for (auto _pair: primes){
      dump(_pair);
      if (!dict.contains(_pair.first)) dict[_pair.first] = _pair.second;
      else dict[_pair.first] += _pair.second;
    }
  }
  dump(dict);
  dump(prime_factorize(543));
  // 2,4,14,24,74
  vec ls(5,0);
  ...
  dump(ls);
  // 4 4 2
  if (ls[1]>=2 & ls[0]>=3) res += ls[1]*(ls[1]-1)/2*(ls[0]-2);
  // 24 2
  if (ls[0]>=1) res += ls[3]*(ls[0]-1);
  // 14 4
  if (ls[1]>=1) res += ls[2]*(ls[1]-1);
  // 74
  res += ls[4];
  cout << res << endl;
}
