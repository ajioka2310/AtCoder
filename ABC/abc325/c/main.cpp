...
vector<int> counter(const vector<int> &v, int max_num = -1) {
    if(max_num == -1) max_num = max_value(v);
...
int main(){
  ...
  VV(char,ls,H,W);
  dump(ls);
  v2i64 check(H,v1i64(W,-1));
  i64 res = 0;
  rep(i,H)rep(j,W){
    if (ls[i][j]=='#'&&check[i][j]==-1){
        res++;
        check[i][j]==res;
        deque<pair<i64,i64>> que;
        que.pb({i,j});
        while(que.size()){
            auto [now_i,now_j] = que.front(); que.ppf();
            fore(idx,idxs)fore(idy,idys){
                if (0<=now_i+idy&&now_i+idy<H&&0<=now_j+idx&&now_j+idx<W){
                    if (ls[now_i+idy][now_j+idx]=='#'&&check[now_i+idy][now_j+idx]==-1){
                        que.pb({now_i+idy,now_j+idx});
                        check[now_i+idy][now_j+idx]=res;
                    }
                }
            }
        }
