...
ll func(ll N, ll X){
  ...
  else if (1<X && X<=A[N-1]+1) return func(N-1,X-1);
  else if (X==A[N-1]+2) return B[N-1] + 1;
  else if (A[N-1]+2<X && X<= 2*A[N-1]+2) return B[N-1]+1+func(N-1,X-2-A[N-1]);
  else if (X==2*A[N-1]+3) return B[N];
}
...
int main(){
  ...
  dump(A |cp::front(10) |cp::dec(2) |cp::index());
  dump(B |cp::front(10) |cp::dec(2) |cp::index());

  dump(A |cp::back(10) |cp::dec(2) |cp::index());
  cout << func(N,X) << endl;
